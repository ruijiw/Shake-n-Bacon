 ...
 * PROJECT 2 PHASE B
 * 
 * HashTable implements the DataCounter interface using a seperate chaining hash table.
 * The constructor takes a Comparator<? super E> and Hasher<E> "function object" so
 * that items of type E can be compared and hashed.
...
public class HashTable<E> extends DataCounter<E> {
	...
	public HashTable(Comparator<? super E> c, Hasher<E> h) {
		primeNum = 0;
		array = (HashNode[]) new HashTable.HashNode[PRIMES[primeNum]];
		comparator = c;
		hasher = h;
		size = 0;
	}
	...
	public void incCount(E data) {
		...
		if(current == null) {
			...
			if (size + 1 > LOAD_FACTOR * array.length && primeNum < PRIMES.length - 1) {
				primeNum++;
				HashNode[] temp = (HashNode[]) new HashTable.HashNode[PRIMES[primeNum]];
